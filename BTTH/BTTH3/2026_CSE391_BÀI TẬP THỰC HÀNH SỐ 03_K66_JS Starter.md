...
Trình duyệt đã đọc và thực thi tệp script: Trình duyệt quét qua file HTML, gặp thẻ <script src="main.js"></script> và ngay lập tức tải tệp JS về để xử lý.
---
...
> a) JavaScript chạy ở đâu? (Trình duyệt / Server / Cả hai?)
  Chạy được ở cả hai (Trình duyệt - Client side và Máy chủ - Server side thông qua Node.js)
> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
>
> - HTML: Xây dựng cấu trúc và nội dung thô của trang web (văn bản, hình ảnh, liên kết)
> - CSS: Phụ trách giao diện, cách trình bày, màu sắc và bố cục để trang web đẹp hơn.
> - JavaScript: : Tạo ra sự tương tác và các hành vi động (xử lý dữ liệu, phản hồi khi click chuột, hoạt ảnh).
...
>   - `string` là: Kiểu dữ liệu chuỗi/văn bản, luôn nằm trong dấu ngoặc kép.
>   - `boolean` là: Kiểu dữ liệu logic, chỉ nhận giá trị true (đúng) hoặc false (sai).
...
quên dấu ";" không sao cả. 
quên dấu + bị lỗi. báo lỗi: ncaught SyntaxError: missing ) after argument list (at main.js:7:13)
...
gõ vào console trong devmode chỉ xthực hiện dòng lệnh cuối, có tự sửa lỗi. có thể gõ để nói chuyện luôn với trang web
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?
gặp lỗi cú pháp gõ nhiều câu lệnh xong mới ấn enter. em tự sửa
...
> - Khi nào chương trình in `"Giỏi"`?
 khi giá trị score lớn hơn 8
> - Khi nào chương trình in `"Yếu"`?
khi giá trị score không thỏa mãn các yêu cầu trên
> - Em hãy mô tả cấu trúc `if/else` bằng lời của em (có thể ví von “ngã rẽ” trong đời sống):
cấu trúc if/else giống như các nhánh trên 1 thân cây nếu ta đi nhánh này sẽ chỉ có thể chọn các nhánh con chứ không thể quay trở lại
...
>   - Tham số (parameters): các giá trị đầu vào của hàm
>   - Thân hàm (body):các câu lệnh nằm trong { }.
>   - Giá trị trả về (return): kết quả mà hàm trả về thỏa mãn điều kiện nào đó của thân hàm
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
nhanh gọn tái sử dụng nhiều lần
...
khác biệt giữa return và console.log. khi gán biến tại sao khi gán biến let avg = (m1 + m2 + m3) / 3; trong console nó lại hiện undefined thay vì hiện số 8.
> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …)?
em đã tham khảo bạn và gõ trực tiếp tên biến vào console để hiện giá trị
---
